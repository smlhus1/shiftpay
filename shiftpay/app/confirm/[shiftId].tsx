...
export default function ConfirmShiftScreen() {
    ...
    async (status: "completed" | "missed" | "overtime") => {
      ...
      if (status === "overtime") {
        ...
        try {
          ...
          Alert.alert(
            t("common.error"),
            e instanceof Error ? e.message : t("confirm.errors.saveError")
          );
        } finally {
          setSubmitting(false);
        }
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    try {
      ...
      setShift((prev) =>
        prev ? { ...prev, date: editDate, start_time: editStart, end_time: editEnd } : prev
      );
      setEditDirty(false);
    } catch (e) {
      Alert.alert(
      ...
      >
        <AnimatedCard index={0}>
          <View className="items-center">
            <View
              className="mb-4 h-16 w-16 items-center justify-center rounded-full bg-emerald-50 dark:bg-emerald-500/15"
            ...
            <PressableScale
              ...
              className="mt-4 rounded-xl border border-app-border bg-app-surface px-6 py-3 dark:border-dark-border dark:bg-dark-surface"
            >
  ...
  return (
    ...
    >
      ...
      >
        <AnimatedCard
          index={0}
          className="mb-6 rounded-xl border border-app-border bg-app-surface p-4 dark:border-dark-border dark:bg-dark-surface"
        >
        ...
        <AnimatedCard index={1} className="mb-4">
          <Text className="mb-2 font-inter-medium text-xs uppercase tracking-wider text-stone-600 dark:text-stone-400">
            {t("confirm.payType.label")}
          </Text>
          <View
            className="flex-row gap-2"
            accessibilityRole="radiogroup"
            accessibilityLabel={t("confirm.payType.label")}
          >
        ...
        <AnimatedCard index={2} className="mb-4">
          ...
          >
            <Text className="font-inter-medium text-sm text-stone-700 dark:text-stone-300">
              {t("confirm.editFields.title")}
            </Text>
            <Icon
              name={showEditFields ? "chevron-up" : "chevron-down"}
              size={18}
            ...
            <View className="mt-2 rounded-xl border border-app-border bg-app-surface p-4 dark:border-dark-border dark:bg-dark-surface">
              <View className="mb-2">
                ...
                <ThemedTextInput
                  ...
                  className="rounded-xl border border-app-border bg-app-elevated px-4 py-3 text-stone-900 dark:border-dark-border dark:bg-dark-elevated dark:text-stone-100"
                />
              </View>
              ...
              {editDirty && (
                <PressableScale
                  ...
                  disabled={savingEdit}
                  className="mt-3 rounded-xl bg-accent-dark py-3 dark:bg-accent"
                  style={savingEdit ? { opacity: 0.6 } : undefined}
                >
                  {savingEdit ? (
                    <ActivityIndicator color={colors.bg} accessibilityLabel={t("common.loading")} />
                  ) : (
                    <Text className="text-center font-inter-semibold text-sm text-white dark:text-stone-900">
                      {t("confirm.editFields.save")}
                    </Text>
                  )}
                </PressableScale>
              )}
        ...
        {!showOvertime ? (
          <AnimatedCard index={3}>
            ...
            <PressableScale
              ...
              accessibilityState={{ disabled: submitting }}
              className="mb-3 rounded-xl border border-app-border bg-app-surface py-4 dark:border-dark-border dark:bg-dark-surface"
              style={submitting ? { opacity: 0.6 } : undefined}
            >
              <Text className="text-center font-inter-medium text-stone-700 dark:text-stone-300">
                {t("confirm.missed")}
          ...
          <>
            ...
            <PressableScale
              ...
              accessibilityState={{ disabled: submitting }}
              className="mb-3 rounded-xl bg-accent-dark py-4 dark:bg-accent"
              style={submitting ? { opacity: 0.6 } : undefined}
            >
              {submitting ? (
                <ActivityIndicator color={colors.bg} accessibilityLabel={t("common.loading")} />
            ...
            <PressableScale
              ...
              className="rounded-xl border border-app-border bg-app-surface py-3 dark:border-dark-border dark:bg-dark-surface"
            >
              <Text className="text-center font-inter-medium text-stone-600 dark:text-stone-400">
                {t("confirm.backBtn")}
              </Text>
            </PressableScale>
          </>
